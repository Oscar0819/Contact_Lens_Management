...
class SignInIntentContract: ActivityResultContract<String, String?>() {
    ...
    // SignInActivity에서 구글 아이디로 로그인 버튼을 누르면 구글 로그인 화면으로 이동하는 intent를 만드는 함수.
    ...
    // 구글 로그인 화면에서 다시 SignInActivity로 돌아올 때 호출되는 함수.
    override fun parseResult(resultCode: Int, intent: Intent?): String? {
        return when (resultCode) {
            // 정상적으로 로그인이 이뤄진 경우 getTokenId 함수를 통해 얻은 tokenId를 SignInActivity에 전달.
